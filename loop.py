...
async def checking(url, sidel, sideh, tabl, tabh, number, max_retries=5):
    retry_count = 0
    api_id = '25126202'
    ...
    async with async_playwright() as p:
        while retry_count < max_retries:
            try:
                ...
                print(f"Retry {retry_count + 1} of {max_retries}: Timeout error encountered. Retrying...")
                retry_count += 1
                message = f"Error during scraping"
                numbee="+919481092916"
                await send_notification(api_id, api_hash, numbee, message)
                await asyncio.sleep(60) 
            
            
        await browser.close()
        message = "Max retries exceeded. Unable to open the site. Try Again."
...
async def scrape_amazon(url, sidel, sideh, tabl, tabh, number):
    ...
    if (sidel is None ) and \
        ...
        (tabh is None ):
        stop_flag = True
        sideval, tabval = await checking(url,sidel, sideh, tabl, tabh, number)
